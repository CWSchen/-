...
class LDA:
    def __init__(self, K, alpha, beta, docs, V, smartinit=True):
        ...
        self.n_m_z = numpy.zeros((len(self.docs), K)) + alpha  # word count of each document and topic 每个文档和主题的字数
        self.n_z_t = numpy.zeros((K, V)) + beta  # word count of each topic and vocabulary 每个主题和词汇的字数
        self.n_z = numpy.zeros(K) + V * beta  # word count of each topic 每个主题的字数
        self.n_m_t = numpy.zeros((len(self.docs), self.V))  # word count of each document and word 每个文档和单词的字数
        self.N = 0
        for m, doc in enumerate(docs):  # enumerate()是python的内置函数，指在字典上是枚举、列举
            self.N += len(doc)
            z_n = []
            for t in doc:
                if smartinit:
    ...
    def likelihoodOfAllWords(self):
        ...
        """
        result = 0.0
        for m in range(self.n_m_z.shape[0]):
            for t in range(self.n_z_t.shape[1]):
                tmp = 0.0
